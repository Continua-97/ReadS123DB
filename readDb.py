...
OUT_DIR = r'C:\Users\Continua Carlton\Desktop\Outs'
...
def readS123db(inDB=IN_DB):
    ...
    # 3 - Submission Error
    # 4 - Inbox
    for row in cur.execute('SELECT name, data, status from Surveys where status = 0 or status = 1 status = 2 or status = 3 status = 4'):
        # print(row)
        print ('-----------------')
        surveyName = row[0]
